...
// ============================================================
// Top-level core as Vivado IP: separate I-BRAM and D-BRAM
// ============================================================
module rv_pl_ip #(
    // Word-address width of your BRAM (e.g. 12 for 4096x32-bit)
    ...
    // Internal active-low reset used by existing pipeline modules
    wire rst_n = resetn;
    ...
    localparam integer WORD_AW = ADDR_WIDTH; // 2048 words
    wire [WORD_AW-1:0] i_word_addr = F_PC[WORD_AW+1:2];    // PC[12:2]
    ...
    // ------------------------------------------------------------
    // Instruction BRAM (Port B) connection
    // IMPORTANT: we clock BRAM port B on the inverted clock so that
    // a synchronous BRAM read behaves like a "same-cycle" read to the
    // rising-edge pipeline (data becomes stable before next posedge).
    // ------------------------------------------------------------
    assign i_clkb  = ~clk;
    assign i_enb   = 1'b1;
    ...
    // --------------------------
    // MA stage: data memory access (D-BRAM Port B)
    // Same clocking trick as I-BRAM: use inverted clock on BRAM port B.
    // This keeps your original single-cycle load timing (from the POV of the
    // rising-edge pipeline) and avoids adding an extra pipeline stage.
    // --------------------------
...
// - Post-redirect PC hold: hold PC at target one cycle so I-BRAM output
//   (which may be one cycle delayed) settles on the target instruction
// ============================================================
module hazard_unit (
    input  wire       clk,
    input  wire       rst_n,
    ...
    // One cycle after a taken branch/jal, hold PC so F_instr (I-BRAM output)
    // has time to become the target instruction (avoids wrong-path instruction
    // being loaded into decode and causing e.g. wrong immediate 0x111 vs 0x444).
    reg took_redirect_r;
    always @(posedge clk) begin
